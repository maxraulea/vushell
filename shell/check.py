...
def basic_tests():
    return [
        ...
        TestGroup('Simple commands', 'simple', 1.0,
            ...
            Test('Arbitrary programs', test_arbitrary_programs),
            Test('Random binary', test_random_binary),
...
class InteractiveShell():
    ...
    def wait_for_prompt(self):
        try:
            ...
            if self.shell.before:
                err += '\nOutput before shell exited:\n%s' % self.shell.before
            raise TestError(err)
...
def run_cmd(cmd, check_rv=False):
    ...
    try:
        ...
        raise TestError("Timeout waiting for command to terminate")
    try:
...
def test_arbitrary_programs():
    # Prevent a student from implementing all command themselves (hopefully)
    bash_cmp('ls')()
    bash_cmp('ls -l')()
    bash_cmp('ls -al')()
    bash_cmp('md5sum shell.c')()
    bash_cmp('stat shell.c')()
    bash_cmp('ptx shell.c')()
...
def platform_fixes():
    ...
    if platform.machine() == 'aarch64':
        # On M1 (inside docker) LSan is super slow and breaks some tests
